...
# Pad the bag arrays to ensure they all have the same length
max_len = max(len(bag) for bag, _ in training)
training = [(bag + [0] * (max_len - len(bag)), output) for bag, output in training]
...
# Use the new learning rate schedules provided by TensorFlow
